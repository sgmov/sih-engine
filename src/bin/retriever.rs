//! retriever 即温故宿主命令面，承接 SPEC-007#interface-signature 与 SPEC-008#boundary。
//!
...
fn main() {
    ...
    if argv.first().map(|s| s.as_str()) != Some("recall") {
        fail("用法 retriever recall --topic 词 --event 记号 --since 日期 --until 日期 --archive 档名 --at 日期 --out 路径", 2);
    }
