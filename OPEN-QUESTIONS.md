...
## 收编归档声明（2026-08-25） {#consolidation}

本档归档为历史工作记忆，不再新增条目。收编批经 facet 测量五发同构反对即归类属治理判断不得由采样承载，裁 facet 拒绝受理该任务类落确定性核对通道，归类权威由机械核验承载即八查报告 sih-tools/proposition/DES/m-oq-consolidation/deterministic-checks.json，裁示见同目录 disposition.md，意图流 round 9 事件 463f9617。未决事项停靠职能由工具线泊界承接即 PARKING-v1 与书简停泊事件，推导职能按本档自规迁移 DEC 与 DES。逐项处置如下，销账条目的正文保留为历史推导。

| 条目 | 处置 | 载体或去向 |
|---|---|---|
| OQ-09 | 销账 | 核阅 scrutinator 于 2026-08-20 承接 doclint 强制位，des-001 包在役；泊界 pk-001 候出泊明令 |
| OQ-11 | 销账 | 泊界承接停靠职能即 PARKING-v1 与停泊事件，忘有警由心跳在役 |
| OQ-21 | 销账 | facet-measure skill 修订一修订二与判据 v3 承载人节点退化 |
| OQ-22 | 销账 | facet 产材料加核阅检词化格链加书简上链管道在役；DES-011 双件在档待验收 |
| OD-01 | 销账 | sih-engine 提交在役 |
| OQ-08 | 过时销账 | facet 自持 key 直连属自建自部署形态在役，开源默认路径结论维持不采用，正式 DEC 不再单独立项 |
| OQ-12 | 并线销账 | 信息洪流一体双面主线即消解载体，主线在役即本条在役 |
| OQ-05 | 入泊 | pk-006 即 worktree 或 lock 组件立项时随 DEC 裁定 |
| OQ-06 | 入泊 | pk-011 即首个 knowledge 产物触发时裁边界 |
| OQ-07 | 入泊 | pk-007 即立名优先级批次经裁 |
| OQ-10 | 入泊 | pk-008 即卸载经裁且治理节适用域改述 |
| OQ-13 | 入泊 | pk-009 即 SPEC-003 修订与五法工程映射经裁 |
| OQ-14 | 入泊 | pk-012 即判死前提被 CHK-6 机械推翻，counter 代码仍在盘面与 facet ROADMAP 物理删除记载不符，counter 处置与度量计算层定位经裁 |
| OQ-16 | 入泊 | pk-010 即三判经裁落档或明确弃置 |
...
- 问题。用户 2026-08-14 裁决：治理流程中的人节点要全部退化为 LLM 点与机器点，审计痕迹为人类参与的唯一残留形态。此决议修订手动阶段「关键定性是人节点」的既有安排，波及范围：AGENTS.md 手动阶段定位、sihankor-facet-measure skill 自动化边界（Layer 2 签核与刀锋改写两处人节点）、facet Layer2_signoff 接口语义、P3 面板 9/9 判定所依据的规则环境。
- 已推导结论。第一，决议与工程基线二/三的既有轨迹同向（人类注意力是稀缺资源，介入点收窄到异常信号），不是方向反转而是加速到位：手动阶段的人节点本是无机器层可用时的降级补位。第二，P3 判定不被推翻而是收窄指称域：节点本身重定义为 LLM 加机器加审计后，LLM 执行该节点不再构成「代签」（无被代者），P3 判定保留为旧规则环境下的越权判定历史。第三，自举边界：本决议自身的批准是一次不可退化的节点（定义委托权限的行为不能由受托方执行），用户当日口头裁决即为该批准，此后操作性节点按新规则运行。
- 待定。第一，审计痕迹的确定性消费器。留痕无消费等于装饰，旧仓死于留痕堆积无人读。须指定机器扫描判据与异常视图触发条件，并判定人在异常告警时是否仍介入。第二，机器签核的强度下限。facet 实测单席位方向不可靠（判据 v3 知识），替代 Layer 2 的形态须至少是池化多席位加机械闸；刀锋改写权移交 LLM 后，防钓鱼须有机械形态，候选方案为改写谱系全量入 trail 且闸看全历史。第三，修订载体。AGENTS.md 手动阶段表述与 skill 自动化边界节须同步修订，是否新增 DEC 承载待定。
- 实证注记。Layer 2 首次真实签核发生在 2026-08-13（rg-P1/P2/P3 三条 promote），样本量为 1。机器形态承接签核职能后，保留并行对照观测一段时期，判定机器签核与既有单例人签的偏离。
- 关联。AGENTS.md 工程基线与禁止条款。sihankor-facet-measure skill 自动化边界节。facet ROADMAP 判据 v3 交付定格节。P3 面板 9/9 判定。
- 阻塞。无。用户裁决已给出，待修订载体落地。
- 哲学依据。工程基线第一条确定性程序是治理操作唯一执行者。工程基线第二条注意力稀缺。工程基线第三条人类只投向异常信号。PRO-08 应而不藏，审计痕迹是构成性条件。

### OQ-22 裁决材料与基线核对二层结构

- 问题。用户 2026-08-14 提出裁决架构定型：facet 范式产出裁决材料，sih-engine 做机器工程基线核对，核对通过则裁决通过，全程留痕审计。此架构是 OQ-21 人节点退化决议的具体承载形态，登记其设计含义与待定项。
- 已推导结论。第一，架构与工程基线一的字面构成一致：facet 是材料生成器（LLM 只生成符号材料），sih-engine 是执行者（确定性程序做核对与写入）。r2 测量违向票指出的「登记写入应由确定性程序执行」在此结构下被正面满足，不依赖规约语料重训。第二，可复用件已存在：facet 锚定白名单与闸 v3 承接来源可追溯与结果可机械校验，event_stream 的 append/hash/verify 承接留痕不可篡改，基线四三性质机件齐备。第三，认识论边界须显式声明：基线核对保证合规性不保证语义正确性，正确性由审计事后追偿与 flywheel 全历史承担；单席位方向不可靠的实测知识（判据 v3）仍是材料合格强度下限的约束，OQ-21 待定二未消解。
- 待定。第一，基线核对规则集的机械化：五条基线当前是散文非规则，须定义每条的可程序化判据（材料 schema 合规、anchor 真实性、哈希链完整、写入者归属、LLM 调用预算上限），载体建议 DES 级设计。第二，facet 闸三态到裁决处置的映射：stable_clear 加核对通过等于裁决通过；near_threshold 与 boundary 的处置（打回、挂起、升级池化）须立映射表。第三，桥接件落位：sih-engine 需一个消费 facet 报告、执行基线核对、写事件流的确定性程序，是参验组件路径一的首个真实实例，同时为 OQ-09 doclint 迁移与 OQ-10 旧仓 MCP 替代提供落点。
- 关联。OQ-21。DES-011 裁决材料基线核对设计（本 OQ 待定一规则集与待定二映射表的首版承载，2026-08-14 起草过 doclint 待用户验收）。DES-006 参验基准预置。SPEC-003 参验规约。SPEC-004 事件流规约。PRO-002 trail 组件关系。facet ROADMAP 判据 v3 交付定格。
- 阻塞。DES-011 待用户验收。桥接件实现前须先完成组件立名 DEC。
- 哲学依据。工程基线一确定性程序唯一执行者。工程基线四可验证性。PRO-07 鉴层检验由可重复程序承载。PRO-08 应而不藏。
