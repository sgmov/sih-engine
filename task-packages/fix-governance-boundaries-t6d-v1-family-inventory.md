...
### 严格分类：v1-family 真函数（按 calls = 15 个，待迁移） {#严格分类-v1-family-真函数-按-calls-15-个-待迁移}

按文件分组：

#### basis_split_heal_probe.py {#basis_split_heal_probe-py}

- `main@basis_split_heal_probe.py:159-233`（reads=verdict, calls=['v1']）

#### cascade_ng_probe.py {#cascade_ng_probe-py}

- `run_condition@cascade_ng_probe.py:158-185`（reads=verdict, calls=['v1']）

#### flywheel_microcircuit_probe.py {#flywheel_microcircuit_probe-py}

- `main@flywheel_microcircuit_probe.py:126-217`（reads=verdict, calls=['v1']）

#### layer2_signoff.py（critical path） {#layer2_signoff-py-critical-path}
...
#### lightweight_mode_probe.py {#lightweight_mode_probe-py}

- `main@lightweight_mode_probe.py:363-498`（reads=verdict, calls=['v1']）

#### maturation_gate.py（v1 闸自身） {#maturation_gate-py-v1-闸自身}
...
#### real_guidance_run.py {#real_guidance_run-py}

- `main@real_guidance_run.py:120-207`（reads=verdict, calls=['v1']）

#### refine_loop_probe.py {#refine_loop_probe-py}

- `main@refine_loop_probe.py:169-287`（reads=verdict, calls=['v1']）

#### reflexive_signing_probe.py {#reflexive_signing_probe-py}

- `main@reflexive_signing_probe.py:103-169`（reads=verdict, calls=['v1']）

### mixed by calls（v1 + v3 跨族，1 个） {#mixed-by-calls-v1-v3-跨族-1-个}

#### layer2_signoff.py（A4 已修） {#layer2_signoff-py-a4-已修}

- `_cmd_route@layer2_signoff.py:229-306`（reads=verdict_v3, calls=['v1', 'v3']）： **A4 修复后已基于 v3 决策，v1 verdict 仅供 audit 保留**

### v2-family by calls（8 个） {#v2-family-by-calls-8-个}
...
### v3-family by calls（5 个） {#v3-family-by-calls-5-个}
...
### reads-only（24 个，决策不依赖仅消费 trail） {#reads-only-24-个-决策不依赖仅消费-trail}
...
### false positive（4 个，custom verdict 命名空间） {#false-positive-4-个-custom-verdict-命名空间}
...
### 1. 高（critical path，必须先迁） {#1-高-critical-path-必须先迁}
...
### 2. 中（重要但非 critical，影响实验/分析） {#2-中-重要但非-critical-影响实验-分析}
...
### 3. 低（仅历史/分析，可立独立任务） {#3-低-仅历史-分析-可立独立任务}
...
### 4. reads-only（24 个，不需迁移代码，仅需评估依赖） {#4-reads-only-24-个-不需迁移代码-仅需评估依赖}
...
### 第一批（critical 路径必须先改） {#第一批-critical-路径必须先改}
...
### 第二批（中优先级，与 v3 闸一致化） {#第二批-中优先级-与-v3-闸一致化}
...
### 第三批（低优先级，旧实验可独立任务） {#第三批-低优先级-旧实验可独立任务}

4. r3a_gate_v2 / r3b_new_props / r5_emphasis_feedback 的历史实验函数

### 第四批（reads-only 评估） {#第四批-reads-only-评估}
