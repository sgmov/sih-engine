# SPEC-023 mcpline alpha 相只读面契约：读数工具 MCP 面契约正典 v1 五具修订一起七具修订二起八具
...
- 读数工具自修订二起八具即 chain_query 与 chain_verify 与 critsweep 与 heartbeat 与 locks_read 加 nomenclator_query 与 nomenclator_check 加 nomenclator_query 与 nomenclator_check 与 retriever_recall；两检词具随 mcpnomgate-solo 批入面，温故检索具随 wengumcp-parallel 批入面；承接路径全是只读子命令，MCP 面零写入零治理语义::[工具契约](#tools)
...
1. alpha 相零写入零治理语义：MCP 面不新增第二个执行者，判定语义正典留确定性程序；各项承接路径全是只读子命令，v1 五项修订一起七项修订二起八项
