...
配对不变量三道机械门。重入拒即同 entry_id 已在泊再 enter 拒。无主出拒即无在泊 enter 的 exit 拒。号源唯一拒即重放面内该 entry_id 出现过任何 parking_entered 即拒，无论当前在泊或已出泊，承 entryunique-solo 修订二 2026-09-03。现存链历史双 enter 即 pk-043 撞号改正案不受影响，门只拦新 enter 请求。在泊判定按链序重放停泊事件，重放面即链目录全量即 --trail 同目录全部 ndjson 按名序，承 parkreplay-solo 修订一 2026-09-03；追加面仍为 --trail 单文件即当日链自身链序，跨天出泊即泊入在先日链、出泊落记录当日链。拒绝零留痕即不入流。
...
T4 park 配对不变量
: 重入拒与无主出拒与号源唯一拒三道门，在泊判定链序重放即链目录全量，出泊后不可再入。
