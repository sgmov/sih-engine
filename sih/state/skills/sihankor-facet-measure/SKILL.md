...
**人节点，agent 不得代办**：
- Layer 2 签核（promote/reject/hold）经 2026-08-29 修订六由机器终签取代：收敛成
  唯一方向由确定性核对（执契 tally，承 DEC-019）落据即终签，不再待人签核。人节点
  收缩到唯一异常：不收敛（near_threshold、boundary、席位异常）时诊断遗漏条件或
  命题错误。agent 代签 = violate（P3 自反裁决 9/9，rg-P3 已签核判定成立，原文
  留痕不抹）——终签主体是确定性脚本，LLM 在任何情况下都不落签名。
- boundary（打回重作）的**上下文重写**：刀锋命题须人重写措辞/上下文后重测，
  agent 自行改写重测 = 调参钓鱼邻域，禁止（此即非收敛诊断的动手形态）
...
4. **汇报**（人话，三句内）：判定方向与稳定性、落进哪个队列、下一步是无需动作
   （stable_clear 已由执契终签落据即重放锚，修订六）还是人诊断（不收敛时：遗漏
   条件还是命题错误）。
...
2026-08-30 修订七，机器签署即为人授权（硬件钥匙模型）。用户裁定：得一／attractor 裁定的人节点也退，机器签署即为人授权，类比硬件密码钥匙点一下即授权；授权的判据是可重复收敛即 stable_clear 加执契核对全过，人节点退至两处即武装（规则、基线、验收标准的供给）与异常诊断；适用界以组件名采纳与已收敛裁定为界，向外扩展须逐类裁定。P3 字面不变且加强即 agent 一字不签，终签主体是确定性脚本；首例即组件名名实相副材料终签 2657ac9b 并经本裁定追认为授权记录，承 DEC-020。本条显式接替修订六的人节点保留语义，修订六与 P3 与 rg-P3 留痕不抹。

2026-08-29 修订六，机器终签取代常设人签核位。用户裁定：facet 的最终目的是限定条件内的唯一方向选择，收敛成一条固定方向时由确定性脚本终签落据；人无法像 LLM 对所有条件都审一遍且有遗忘与上下文压力，常设人签核位实际只会是橡皮图章或瓶颈，故去之；人节点收缩到唯一异常即不收敛时诊断遗漏条件或命题错误；机器落据带全条件绑定即为条件改变时的重制点，重放即可不必重审，异向则翻案留痕；换人换模型不崩，审查由当事人依赖改为重放依赖。执行者即执契（tally，DEC-019 外切壳），落据调用形即 sihankor-tally 调用壳之 assemble→check→verify→sign 四步。P3 原裁决字面不变且加强：agent 仍一字不签，终签主体是确定性脚本；本条显式接替修订二「签认节点仅存于 stable_clear」的常设位，P3 与 rg-P3 留痕不抹。
