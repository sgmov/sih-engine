...
- boundary（打回重作）的**上下文重写**经 2026-08-30 修订九由 agent 承载：
  改写强制新 gid 加谱系披露、方向须对己不利并走通道分解，同命题改写超
  三次自动告警（R7 机械护栏）；人节点退出命题生命周期，整条改写链可见
  可数可否决
...
2026-08-30 修订九，命题全生命周期人节点退出。用户裁定：使用流程第 1、2 步（命题起草与出合同）从头到尾都是 LLM 在做，包括命题打回后的修改，人从未写过——该人节点也退出。起草、出题、打回后改写全由 agent 承载；防钓由机械护栏承载即 R7 三判据（新 gid、谱系披露、改写超三次告警）加修订一对己不利方向与通道分解，改写链可见可数可否决，人保留武装位、一票绝对反对权、知晓权；本裁定为 DEC-020 逐类裁定条款的命题生命周期类首例。显式接替修订一的人重写兜底与自动化边界的 boundary 人重写禁令，留痕不抹；P3 不变即 agent 一字不签终签位，起草改写是材料生成非签署。

2026-08-30 修订八，签署节点三条款。用户裁定：一，签署后明确输出让人知晓，零动作要求、无需复签；二，人类一票绝对反对权，否决经 inconsistency 翻案事件覆盖被签效力，签名留痕不抹；三，级联边界变更（条件绑定经复算不符）即签署点异常视图，人介入可自重命题或令 agent 重跑新命题。机械面即 tally watch 批量复检与 sign 明确输出行，DES-011 输出契约同批修订（裁决通过进知晓视图，废止仅记录不进视图旧文）。本条为修订六与七的视图与反对权细则，不改动其授权模型。
