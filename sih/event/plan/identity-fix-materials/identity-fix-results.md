...
- SDDG 首拦补录（如实留痕不抹）：SDDG-3 判偏差条目零承载词形、SDDG-4 判差分缺 tests 面文件，两判均属实——按 DEC-024 三通道择补申报通道，补 tests/identity_salt.rs 端到端回归（15 秒超时闸，旧病回流即红不挂套件）并修订本节承载词形后复约，未动显式绕行。
