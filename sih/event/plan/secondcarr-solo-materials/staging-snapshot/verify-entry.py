#!/usr/bin/env python3
"""复归段补强波条目草稿五件机械验收。用法：python3 verify-entry.py  退出码 0=全绿 1=有失败

五件核心即标题前缀、首节名、括号零、破折号零、每条引文 grep -F 逐字节命中且行号复核。
附核即 front matter 可解析、围栏代码块零、引文无全角括号与破折号、结构八节在场。
只报不判，零写路径，同参双跑逐字节一致。

承 agent-drafts/entry/verify-entry.py 扩展：标题前缀白名单扩为 des-001-mathe 0.3.0
全十四族即 TOP PROB ORD ALG CALC LIM DIFF INT APP HIS MUL NS SER SPEC。
"""
import re, sys, pathlib, yaml

ROOT = pathlib.Path(__file__).resolve().parents[2]
STAGING = pathlib.Path(__file__).resolve().parent
ID_PREFIXES = ('TOP-', 'PROB-', 'ORD-', 'ALG-', 'CALC-', 'LIM-', 'DIFF-',
               'INT-', 'APP-', 'HIS-', 'MUL-', 'NS-', 'SER-', 'SPEC-')
