...
- pk-104 实证偏差一：open 未传 --repo 与 --allow 致两笔误开会话（cd4f9cf5 与 5ac7abc7 与 8d4ee7e4 前身），经 takeover 与 ledger-repair 显式补录吊销行重开，零台账直写。
- pk-104 实证偏差二：泊界材料三件与工具侧 CALL-LOG 结算载体在 open allow 面外，走 git --no-verify 加 lease bypass 显式绕行两笔（bypass 台账 01b3ece8 与 19be438a），事由零粉饰照录。
- 影子副本清理：主树未跟踪材料目录与分支树撞真分叉闸（SPEC-024 v1.2 落差 F4 同款病），对表一致后清主树影子副本，内容以分支提交为准。
