...
主会活体验收：rev2 四件在位、三态读数复核成立、F-3 承继位缺口申报判伪——calculus 子仓条目实居 llm-friendly-build/entries/（目录约定例外），LIM-007 磁盘实存，非数学仓数据缺口；summary-rev2.md 已更正（该行改实存加更正注）。其余读数全数对上。经验教训：盘核脚本对子仓目录约定须枚举实态不作统一假设。
