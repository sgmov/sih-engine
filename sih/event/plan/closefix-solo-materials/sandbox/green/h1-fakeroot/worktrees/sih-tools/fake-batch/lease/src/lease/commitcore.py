...
def _load_all_event_hashes(trails):
    """链上全事件哈希集即任意 event_hash 全值（直改链笔引用核验面）。

    直改链笔引用指向任意 scribe pen 落链事件（非认证专属），承 idenlane-guard-solo
    T-8 即 ok_direct 形提交的引用在链认账面。
    """
    hashes = set()
    for trail in trails:
        for event in load_ledger(trail):
...
def _direct_pen_on_chain(body, all_hashes):
    """直改链笔引用在链核验：挂接形命中且哈希前缀对上链上任一事件即真。

    挂接形由 guardcore.DIRECT_PEN 单源认定（形不符不认直改车道），本函数只做
    引用与链的对表，返回命中前缀或 None。
    """
    from lease.guardcore import DIRECT_PEN

    match = DIRECT_PEN.search(body)
    if not match:
        return None
...
def reconcile(repo, ledger, trails, base=None, limit=0, bypass_ledger=None):
    ...
    直改车道路由认账（idenlane-guard-solo T-8）：挂接链笔引用在链的 ok_direct
    形提交归 routed_direct 类计入 routed 不告警，引用不在链落 bypass 对表即
    fail-visible。既有 SEAL 逻辑零动即 exempts 与封线起算全承旧约。
    """
    session_ids, packages = _known_sessions(ledger)
    trail_hashes = _load_trail_hashes(trails)
    all_event_hashes = _load_all_event_hashes(trails)
    ...
    for raw in out.split("\x1e"):
        ...
        elif _direct_pen_on_chain(body, all_event_hashes) is not None:
            # 直改车道路由认账（T-8）：ok_direct 形提交挂接链笔引用在链即视 routed，
            # 不再强制 bypass 例外登记。引用不在链的挂接形不入此类，落 bypass 对表
            # 即已登记归 bypass、未登记出 unbypassed 告警（fail-visible）。
            classification = "routed_direct"
            detail = _direct_pen_on_chain(body, all_event_hashes)
    ...
    summary = {
        ...
        "routed": sum(1 for r in records if r["class"] in ("routed", "routed_merge", "routed_direct")),
        "routed_direct": sum(1 for r in records if r["class"] == "routed_direct"),
