...
归类对表（对 rev1 生效面）：命名常数分类变动 0 件、rev3 新增 17 件、rev1 有而 rev3 无 0 件；比较位字面量变动 0 件、新增 49 件、消失 1 件。逐件细目落 ledger-rev3.json classification.diff_vs_rev1 节，零静默。rev2 计数对表落 classification.rev2_counts_comparison 节。
...
语料内非 SPEC 白名单 ID 命中全部被载体映射覆盖，漏项核对 findings 1 笔；SPEC 双义 ID 不在漏项核对域，消歧规则与规格引用面归属零变动承 rev1。非白名单占位号（wikirecall 自测件 APP-999 与 ORD-905 与 TOP-901 与 TOP-902 等）不在漏项核对域，如实记。

## 口径三分列（缺陷②防误报位）{#caliber}

- 白名单正身号全集 174（calculus 114 + order 21 + probability 19 + topology 8 + algebra 12）
- 磁盘条目文件 174 件（实态目录枚举：calculus 114 件居 calculus/llm-friendly-build/entries；order 21 件居 order/entries；probability 19 件居 probability/entries；topology 8 件居 topology/entries；algebra 12 件居 algebra/entries）
