...
- 双跑对表偏差：接线前后 MCP tools/list 走双端口临时面（8765 在役旧码对 8766 工地新码）同参对表，SPEC-024 同参双跑形条款 alias 域差异如实登记，指纹逐字节一致。
- 施工面残留偏差：scrutinator 测试残留件误入暂存，验收时剔除删除，处置通道承 SPEC-024 v1.2 差集闸先例，非本批产物零入库。
- 环境红偏差：lib 余败与主树同集即 scrutinator 金向量两件与 SPEC-024 worktree 依赖型测试，既有环境红非接线引入，主树对照坐实，候后批处置。
