...
def _merge_files(repo, base, branch):
    """归并面即 base..branch 差集件清单。"""
    rc, out, _ = _git(repo, "diff", "--name-only", base, branch)
    if rc != 0:
        return []
...
def _chain_union_yield(repo, base, branch, files, backup_dir):
    ...
    落盘内容先备份至 deterministic 路径再 checkout 让位，归并后重读
    合并件算补笔清单即备份独有行哈希，账目随 removed 记录入台账事件
    可对账，链语义修复归书简补笔通道不归 close 直写。
    """
    report = {}
...
def _close_precondition(session):
    ...
    for entry in session["repos"]:
        ...
        rc, out, _ = _git(repo, "diff", "--name-only", base, branch)
        merge_files = [f for f in out.splitlines() if f.strip()]
...
def close_session(package_value, force, reason, at, root, ledger, locks_ledger=None):
    ...
    # SPEC-020 步骤二后置：让位放行/归并后，git add -A stage 全部 working tree
    # 但未跟踪文件不能 stage 后 merge — 需要先 git stash 起来（合并完再 pop）
    # 重要：stash -u 包含 .git/MERGE_HEAD 等会被误删，需要先备份 .git 关键标记
    for entry in session["repos"]:
        repo = entry["repo"]
        # 备份 .git 关键标记文件（stash -u 会删）
        git_dir_for_repo = Path(repo) / ".git"
        git_marks_backup = {}
        if git_dir_for_repo.is_dir():
            for mark in ("MERGE_HEAD", "CHERRY_PICK_HEAD", "REVERT_HEAD"):
                p = git_dir_for_repo / mark
                if p.exists():
                    git_marks_backup[mark] = p.read_text(encoding="utf-8")
        # 先看是否有未跟踪文件
        ...
        # 排除 .git 内部文件（避免 stash 把 .git/MERGE_HEAD 删了）
        untracked_paths = [p for p in untracked_paths if not p.startswith(".git/") and p != ".git"]
        has_untracked = bool(untracked_paths)
        if has_untracked:
            # 只 stash 显式列出的非 .git 路径
            args_stash = ["stash", "push", "-u", "-m", "closeguard-solo pre-close stash", "--"]
            args_stash.extend(untracked_paths)
            _git(repo, *args_stash)
            entry["_post_merge_stash"] = True
        # 把 working tree 修改 commit 到主树（close 通道外 commit，bypass 登记）
        # 这样 git merge 不会因 working tree 撞而失败
        # 但仅在 merge_conflicts 全部为 same/append 时做（真分叉应直接拒）
        all_safe = all(
            mc.get("conflict_type") in ("same", "append")
            for mc in merge_conflicts
            if mc["repo"] == entry["repo"]
        ) if any(mc["repo"] == entry["repo"] for mc in merge_conflicts) else True
        has_modifications = any(
            line.strip() and not line.strip().startswith("??")
            for line in out_st.splitlines()
        )
        if all_safe and (has_modifications or has_untracked):
            _git(repo, "add", "-A")
            _git(repo, "-c", "core.hooksPath=/dev/null", "commit", "-m", f"closeguard-solo: pre-close working tree commit ({stem})")
        # 还原 .git 关键标记
        for mark, content in git_marks_backup.items():
            p = git_dir_for_repo / mark
            if not p.exists():
    ...
    for entry in session["repos"]:
        ...
        if worktree_path.exists():
            ...
            # SPEC-020：merge 成功后 pop stash（如果有）
            if entry.get("_post_merge_stash"):
                _git(entry["repo"], "stash", "pop")
        else:
...
def detect_merge_conflicts(repo_path, base_branch, branch):
    ...
    # 获取归并面（base..branch 差集）
    rc, out, _ = _git(repo_path, "diff", "--name-only", base_branch, branch)
    merge_files = [f.strip() for f in out.splitlines() if f.strip()] if rc == 0 else []
    ...
    # 收集真脏位且在归并面中的文件；净态差集件直接退场（F-1 修复核心）
    for f in merge_files:
        if f not in dirty_set and f not in untracked_set:
            continue
        ...
        # 检查是否未跟踪碰撞
        untracked_collision = f in untracked_set
        
        # 未跟踪碰撞：未跟踪文件与 branch 路径碰撞
        if untracked_collision:
            # 但 ondisk == branch_content 时走 same（同内容让位放行），不视为冲突
            if branch_content is not None and ondisk == branch_content:
                # 同内容让位放行（即使是 untracked）
                continue  # 不加入 conflicts，后续 same 路径会处理
            conflicts.append({
                "path": f,
        ...
        # 只有当 ondisk 存在且不等于 branch_content 时，才视为脏位
    ...
    # 添加未跟踪但不在归并面的文件（需检查是否与分支路径碰撞）
    for f in untracked_files:
        if f not in merge_files:
            # 检查该未跟踪文件是否与分支路径碰撞
            rc_b, blob_b, _ = _git(repo_path, "show", f"{branch}:{f}")
            if rc_b == 0:
                conflicts.append({
...
def allow_and_merge(repo_path, base_branch, branch, files_to_merge, untracked_same_files=None):
    """allow_and_merge：让位归并逻辑。
    
    纯追加形文件备份后 checkout 让位，再执行 merge。
    同内容未跟踪文件需要先移除再 merge。
    ...
    # 然后 checkout 让位：把 working tree 文件强制设为 branch 内容（git checkout <branch> -- <f>）
    # 注意：git checkout -- <f> 是从 HEAD 还原（错误语义），应改为 git checkout <branch> -- <f>
    for f in files_to_merge:
        try:
            # 从 branch 复制到 working tree
