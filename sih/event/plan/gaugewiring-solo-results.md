...
偏离一：租约版本随接线升 1.8.0 即行为增面非纯修、CONTRACT 修订十五随档。
偏离二：任务包请求写入漏 sih-tools/lease/pyproject.toml 一行、结算时租约范围闸拦 staged_out_of_scope 即机械闸按设计工作、本会话改收在范围内件、版本三源中 pyproject 暂留 1.7.1、包修正后第二程小会话补齐 1.8.0 三源对齐。
偏离三：租约摘要接线随本批归并生效、本批 open 与 commit 仍走主线旧码即会话档无 gauge 字段、次程起 open 即附三维摘要。期票一：例行读数周期化入章程批待用户另令。期票二：ga-1 公式真数据校准升版随例行读数积累另批。单盲注：open 档 gauge 字段使台账行含读数值即时间弱相依、承 issued_at 先例不增不确定性。
